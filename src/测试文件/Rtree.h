...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, 
         class ELEMTYPEREAL = ELEMTYPE, int TMAXNODES = 12, int TMINNODES = TMAXNODES / 2>
class RTree
{
protected: 
...
public:
  ...
  bool OpenRead(const char* a_fileName)
  {
    ...
    m_file = fopen_s(&stream,a_fileName, "r");
    if(!m_file)
    {
      return false;
    }
  ...
  {
    m_file = fopen_s(&stream,a_fileName, "wb");
    if(!m_file)
    {
      return false;
    }
...
bool RTREE_QUAL::Overlap(Rect* a_rectA, Rect* a_rectB)
{
  ...
  for(int index=0; index < NUMDIMS; ++index)
  {
    if (a_rectB->m_min[index] > a_rectA->m_max[index] ||
  ...
  else // This is a leaf node
  {
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        ...
        if(&a_resultCallback)
        {
		  similar_image[a_foundCount] = a_node->m_branch[index].m_rect.rect_id;
          ++a_foundCount;
